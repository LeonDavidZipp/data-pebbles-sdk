...
from data_pebbles.client.api.bronze import (
	bronze_create_resource as _bronze_create,
)
from data_pebbles.client.api.bronze import (
	bronze_delete_resource as _bronze_delete,
)
from data_pebbles.client.api.bronze import (
	bronze_get_resource as _bronze_get,
)
from data_pebbles.client.api.bronze import (
	bronze_list_resources as _bronze_list,
)
from data_pebbles.client.api.bronze import (
	bronze_list_versions as _bronze_list_versions,
)
from data_pebbles.client.api.bronze import (
	bronze_update_resource as _bronze_update,
)
from data_pebbles.client.api.gold import (
	gold_create_resource as _gold_create,
)
from data_pebbles.client.api.gold import (
	gold_delete_resource as _gold_delete,
)
from data_pebbles.client.api.gold import (
	gold_get_resource as _gold_get,
)
from data_pebbles.client.api.gold import (
	gold_list_resources as _gold_list,
)
from data_pebbles.client.api.gold import (
	gold_list_versions as _gold_list_versions,
)
from data_pebbles.client.api.gold import (
	gold_update_resource as _gold_update,
)
from data_pebbles.client.api.projects import (
	create_project as _project_create,
)
from data_pebbles.client.api.projects import (
	delete_project as _project_delete,
)
from data_pebbles.client.api.projects import (
	get_project as _project_get,
)
from data_pebbles.client.api.projects import (
	list_projects as _project_list,
)
from data_pebbles.client.api.projects import (
	update_project as _project_update,
)
from data_pebbles.client.api.raw import (
	raw_activate_version as _raw_activate,
)
from data_pebbles.client.api.raw import (
	raw_create_resource as _raw_create,
)
from data_pebbles.client.api.raw import (
	raw_delete_resource as _raw_delete,
)
from data_pebbles.client.api.raw import (
	raw_delete_version as _raw_delete_version,
)
from data_pebbles.client.api.raw import (
	raw_get_resource as _raw_get,
)
from data_pebbles.client.api.raw import (
	raw_list_resources as _raw_list,
)
from data_pebbles.client.api.raw import (
	raw_list_versions as _raw_list_versions,
)
from data_pebbles.client.api.raw import (
	raw_update_resource as _raw_update,
)
from data_pebbles.client.api.silver import (
	silver_create_resource as _silver_create,
)
from data_pebbles.client.api.silver import (
	silver_delete_resource as _silver_delete,
)
from data_pebbles.client.api.silver import (
	silver_get_resource as _silver_get,
)
from data_pebbles.client.api.silver import (
	silver_list_resources as _silver_list,
)
from data_pebbles.client.api.silver import (
	silver_list_versions as _silver_list_versions,
)
from data_pebbles.client.api.silver import (
	silver_update_resource as _silver_update,
)
from data_pebbles.client.client import AuthenticatedClient, Client
from data_pebbles.client.models.create_project_request import CreateProjectRequest
from data_pebbles.client.models.create_project_response import CreateProjectResponse
...
from data_pebbles.client.models.update_project_request import UpdateProjectRequest
...
class RawLayer:
	...
	def create_resource(self, name: str, project_id: int) -> int:
		...
		Returns:
			int: The ID of the newly created raw resource.
		"""
		result = _raw_create.sync_detailed(
			client=self._client,
	...
	def list_resources(self) -> list[MetadataResponse]:
		...
		Returns:
			list[MetadataResponse]: Metadata for every raw resource.
		"""
		result = _raw_list.sync_detailed(client=self._client)
		if isinstance(result.parsed, list):
			return result.parsed
		return []

	def get_resource(self, resource_id: int) -> MetadataResponse:
		"""Get metadata for a single raw resource.

		Args:
			resource_id (int): ID of the raw resource.

		Returns:
			MetadataResponse: The resource metadata.
		"""
		result = _raw_get.sync_detailed(resource_id=resource_id, client=self._client)
		if isinstance(result.parsed, MetadataResponse):
			return result.parsed
		raise ValueError(f"Unexpected response: {result.parsed}")

	def update_resource(self, resource_id: int, name: str) -> int:
		"""Rename a raw resource.

		Args:
			resource_id (int): ID of the raw resource to update.
			name (str): New name for the resource.

		Returns:
			int: The ID of the updated resource.
		"""
		result = _raw_update.sync_detailed(
	...
	def delete_resource(self, resource_id: int) -> None:
		"""Delete a raw resource and all its versions.

		Args:
			resource_id (int): ID of the raw resource to delete.
		"""
		_raw_delete.sync_detailed(resource_id=resource_id, client=self._client)

	def list_versions(self, resource_id: int) -> list[VersionResponse]:
		"""List all versions of a raw resource.
		...
		"""
		result = _raw_list_versions.sync_detailed(
	...
	def upload(
		...
		*,
		...
		"""Upload a DataFrame/LazyFrame to a raw resource as IPC stream.

		The data is serialised via ``write_ipc_stream`` and posted to
		the ``/raw/{resource_id}/versions`` endpoint.

		Args:
			resource_id (int): ID of the raw resource to upload to.
	...
	def upload_file(
		...
		*,
		...
		"""Upload a file to a raw resource.
	...
	def download(self, resource_id: int, *, version: int | None = None) -> bytes:
		"""Download a raw version as raw bytes.
	...
	def _latest_version(self, resource_id: int) -> int:
		...
		if not versions:
			raise ValueError(f"No versions found for raw resource {resource_id}")
...
class BronzeLayer:
	...
	def list_versions(self, resource_id: int) -> list[LineageResponse]:
		...
		Returns:
			list[LineageResponse]: Lineage metadata for each version.
		"""
	...
	def upload(
		...
		*,
		...
		"""Upload a DataFrame/LazyFrame to a bronze resource as Parquet.

		Lineage is tracked via *from_resource_id* (the originating raw
		resource).
		...
		Args:
			...
			df (pl.DataFrame | pl.LazyFrame): The data to upload. A
				LazyFrame is collected before serialisation.
			from_resource_id (int): ID of the originating raw resource
	...
	def download(self, resource_id: int, *, version: int | None = None) -> pl.LazyFrame:
		"""Download a bronze version as a LazyFrame.
...
class SilverLayer:
	...
	def list_resources(self) -> list[MetadataResponse]:
		...
		Returns:
			list[MetadataResponse]: Metadata for every silver resource.
		"""
...
class GoldLayer:
	...
	def list_resources(self) -> list[MetadataResponse]:
		...
		Returns:
			list[MetadataResponse]: Metadata for every gold resource.
		"""
...
class DataPebbles:
	...
	Usage::
			...
			# Raw: upload raw files
			dp.raw.create_resource("raw_sales", project_id=1)
			dp.raw.upload_file(1, file_path="sales.csv")

			# Bronze: cleaned tabular data (Parquet)
			dp.bronze.upload(1, df, from_resource_id=1)
			...
			@dp.silver_transform(target_id=3, from_bronze_id=2)
			def clean(lf: pl.LazyFrame) -> pl.LazyFrame:
					return lf.filter(pl.col("x") > 0)


			parse()   # runs on latest raw version
	...
	def __init__(self, base_url: str, *, token: str | None = None) -> None:
		...
		self._raw = RawLayer(self._client)
	...
	def bronze_transform(
		...
		*,
		...
		"""Decorator: raw → bronze with automatic lineage tracking.

		The decorator downloads the raw data, parses it into a
		``LazyFrame``, and passes it to the decorated function.

		By default the file extension is detected from the ``s3_key`` of
		the raw version.  Pass *file_type* to override (e.g. when the
		...
		*target_id* bronze resource, recording *from_raw_id* as the
		originating raw resource.

		Args:
			target_id (int): ID of the bronze resource to upload the result to.
			from_raw_id (int): ID of the raw resource to download input
				from. Also recorded as the lineage origin.
			source_id (int | None): Optional ID to record as the lineage
				origin instead of *from_raw_id*.
		...
		argument to pin a specific raw version (defaults to latest).
		...
		Usage::
				...
				def parse_eu(lf: pl.LazyFrame) -> pl.LazyFrame:
					return lf.filter(pl.col("x") > 0)
		"""
		...
		def decorator(
			...
			def wrapper(*, version: int | None = None) -> None:
				if version is None:
					version_ = self.raw._latest_version(from_raw_id)
				else:
					version_ = version

				versions = self.raw.list_versions(from_raw_id)
				...
				raw_data = self.raw.download(from_raw_id, version=version_)
				lf = _read_bronze_bytes(raw_data, ext, read_options=read_options)
				result = func(lf)
				record_from_id = source_id if source_id is not None else from_raw_id
				self.bronze.upload(target_id, result, from_resource_id=record_from_id)
	...
	def silver_transform(
		...
		*,
		...
		The decorator downloads the bronze data (Parquet) as a
		``LazyFrame`` and passes it to the decorated function.
		...
		*target_id* silver resource, recording *from_bronze_id* as the
		...
		def decorator(
			...
			def wrapper(*, version: int | None = None) -> None:
				...
				lf = self.bronze.download(from_bronze_id, version=version_)
				result = func(lf)
